...
public static class ExecuteProcess
{
    ...
    public static void Execute(List<Vector3> PosList)//'PosList' is the isolated array of positions as xyz values, as they appear in the dataset's CSV
    {
        ...
        LocalDensity(PosList); // a custom function
    }
    ...
    public static void ReturnResults()
    {
        ...
        LumeRuntime.API.ShowResult("Density: " + Result); //creates a panel with 'Result' value on it in Lume application
    }

    //'LocalDensity' is an example of a custom function. It calculates the density value for data in a volume. 
    static void LocalDensity(List<Vector3> PosList)
    {
        int n = PosList.Count;
